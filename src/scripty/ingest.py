...
def ingest_frames(project: str, scene: str, take: str, frames: list[Frame], workers: int = 6) -> dict:
    ...
    with cf.ThreadPoolExecutor(max_workers=workers) as ex:
        for fut in cf.as_completed([ex.submit(one, f) for f in frames]):
            ...
            inv_rows.extend(rows_from_inventory(inv, project=project, scene=scene, take=take, shot=f.shot, t_s=f.t_s, frame_id=fid))
    db.ensure_schema()
