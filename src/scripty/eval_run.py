...
def run(video: Path, project: str, frames_dir: Path, out_dir: Path, max_scenes: int = 40, every_s: float = 3.0, max_per_shot: int = 3, workers: int = 6) -> dict:
    t0 = time.time()
    ...
    for sc in scenes:
        ...
        per_scene.append({"scene": scene, "start_s": sc.start_s, "end_s": sc.end_s, "shots": len(sc.shots), "frames_A": len(a_frames), "labels": lab, "recall_hits": sum(hits), "planted_positives": len(planted_pos), "planted_tp": tp, "control_positives": len(control_pos), "within_A_error_rate": (sum(1 for f in within_a if f["verdict"] == "continuity_error") / len(within_a)) if within_a else None, "findings": findings})
