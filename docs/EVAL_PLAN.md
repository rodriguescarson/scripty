...
| 3 | Flopped shots surfaced only as one entity's coarse position change, which the verifier called `same` (confidence 1.0). Three model-based fixes were tried first and failed the same way: the anchored inventory echoes the sheet's left/right values on mirrored frames (15 of 16 repeated), an independent left-to-right listing aligned too few items, and a direct pairwise question got `same` from Gemini 2.5 Flash on 14 of 15 pixel-verified mirrored pairs — **the model is mirror-blind**. | 8 flipped shots, 3 position candidates, 0 confirmed; 14/15 mirrored pairs judged `same`. | Deterministic mirror test: mean absolute difference of the other take's frame against the reference and against the mirrored reference (blurred 320×180 grayscale); a frame far closer to the mirrored reference is a flop. Reported as `verified = 0` because the verifier is not consulted. |
| 5 | `ingest_frames` kept a list of failed inventory calls but nothing printed or counted it, and the frames table only received frames whose call succeeded. Across the first seven run-1 scenes only **132 of 318** CONTROL/PLANTED frames were inventoried (41 %); a planted change on a dropped frame can never be found. Likely cause: the anchored prompt plus a 4,096-token output cap truncating the JSON. | scene049: 0 of 24 PLANTED frames; scene030: 20 of 36. | Run 2 ingests every planted frame from disk, raises the output cap to 16,384 tokens in sheet mode, and reports the failure count per scene. |
